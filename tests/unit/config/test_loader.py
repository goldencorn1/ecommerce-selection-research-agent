...
def test_load_yaml_config_reads_utf8_chinese(tmp_path):
    config_path = tmp_path / "utf8.yaml"
    config_path.write_text("说明: 中文配置\n模型: deepseek-v4-flash\n", encoding="utf-8")

    config = load_yaml_config(str(config_path))

    assert config["说明"] == "中文配置"
    assert config["模型"] == "deepseek-v4-flash"
