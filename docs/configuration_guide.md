...
For the DeerFlow tool-calling workflow, choose a model variant that supports tool calls. DeepSeek's OpenAI-compatible API can be selected explicitly with `platform: deepseek`; use the reasoning variant only for roles that need it. Additionally, all Gemma-3 models are currently unsupported due to the lack of tool usage capabilities.

### Supported Models

`doubao-1.5-pro-32k-250115`, `gpt-4o`, `qwen-max-latest`,`qwen3-235b-a22b`,`qwen3-coder`, `gemini-2.0-flash`, `deepseek-v4-flash`, and theoretically any other chat models that implement the OpenAI API specification.
...
```yaml
# Use DeepSeek V4 Flash to handle basic tasks
BASIC_MODEL:
  platform: deepseek
  base_url: https://api.deepseek.com
  model: "deepseek-v4-flash"
  api_key: YOUR_API_KEY
# Optional: use DeepSeek V4 Pro for reasoning tasks
REASONING_MODEL:
  platform: deepseek
  base_url: https://api.deepseek.com
