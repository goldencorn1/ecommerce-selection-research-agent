...
def _get_deepseek_env_conf() -> Dict[str, Any]:
    """Read provider-level DeepSeek settings from environment variables."""
    ...
    return {target: os.environ[name] for name, target in aliases.items() if os.getenv(name)}
...
def _create_llm_use_conf(llm_type: LLMType, conf: Dict[str, Any]) -> BaseChatModel:
    ...
    configured_platform = str(merged_conf.get("platform", "")).lower()
    if configured_platform in {"deepseek", "deepseek_api", "deepseek-api"}:
        # Provider-level aliases are convenient for a shared DeepSeek setup;
        # role-specific BASIC_MODEL__/REASONING_MODEL__ values still win.
        merged_conf = {**llm_conf, **_get_deepseek_env_conf(), **env_conf}
    ...
    is_deepseek = platform in {"deepseek", "deepseek_api", "deepseek-api"}

    # DeepSeek exposes an OpenAI-compatible API, but using its native LangChain
    # integration avoids routing DeepSeek-specific requests through the generic
    # OpenAI class.  Keep this branch before the reasoning-model branch so both
    # BASIC_MODEL and REASONING_MODEL can explicitly select DeepSeek.
    if is_deepseek:
        deepseek_conf = merged_conf.copy()
        deepseek_conf.pop("platform", None)
        deepseek_conf["api_base"] = deepseek_conf.pop("base_url", "https://api.deepseek.com")
        api_key = deepseek_conf.get("api_key")
        if isinstance(api_key, str) and not api_key.isascii():
            raise ValueError("DeepSeek API key must contain only ASCII characters; check for a placeholder or non-ASCII space")
        return ChatDeepSeek(**deepseek_conf)
